...
class BuyerController extends Controller
{
    ...
    {
        $bids = Buyer::where('buyers.user_id', '=', Auth::id())
            ->where('buyers.product_id', '=', $request->product_id)
            ->get();
        ...
        if (count($bids) == 0) {
            if (Auth::check()) {
                ...
                Notification::send(
                    $userSchema,
                    new SellerNotication($offerData)
                );
                ...
                return back()->with(
                    'success',
                    'Your bid is successful! Go to the dashboard'
                );
            }
            // user doesn't exist
        } else {
    ...
    public function UpdateSubmitBidFinally(Request $request, $id)
    {
        if (Auth::check()) {
            ...
            $user = Buyer::where('user_id', Auth::id())
                ->where('product_id', '=', $id)
                ->update([
                    'name' => $request->name,
